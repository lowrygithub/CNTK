...
namespace CNTK
{
                                ...
                                std::unordered_map<Variable, Variable>& placeholderReplacements)
    {
        ...
        else
        {
            ...
            auto clonedComposite = blockFunction->Composite()->Clone(parameterCloneMethod, replacements);

            auto cloneeBlockCompositeArguments = blockFunction->Composite()->Arguments();
            ...
            auto cloneeBlockCompositeArgumentsMap = blockFunction->CompositeArgumentsMap();
            ...
            clonedFunction = MakeSharedObject<BlockFunction>(std::move(clonedComposite), clonedBlockCompositeArgumentsMap, blockFunction->OpName(), std::move(attributesCopy), blockFunction->Name());
        }
    ...
    {
        bool isComposite = (composite->RootFunction() != composite);
        if (!isComposite)
