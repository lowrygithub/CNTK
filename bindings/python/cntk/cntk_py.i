...
%typemap(directorargout)
    // Swig would create this conversion for the 'const' variants as well, which
    // we do not want. Therefor, we have to explicitly tell it for which ones it should do it.
    std::unordered_map<CNTK::Variable, CNTK::ValuePtr>& dir_outputs,
    std::unordered_map<CNTK::Variable, CNTK::ValuePtr>& dir_backPropagatedGradientValuesForInputs
{
    // directorargout
    ...
    for (auto it: $1)
    {
        ...
        while (PyDict_Next($input, &pos, &py_key, &py_value)) {
            void *cntk_key = 0;
            int key_res = SWIG_ConvertPtr(py_key, &cntk_key, SWIGTYPE_p_CNTK__Variable, 0);

            CNTK::Variable* cntk_var = reinterpret_cast<CNTK::Variable*>(cntk_key);
            if (*cntk_var == it.first)
            {
                void *cntk_val = 0;
                int val_res = SWIG_ConvertPtr(py_value, &cntk_val, SWIGTYPE_p_std__shared_ptrT_CNTK__Value_t, 0);
                if (!SWIG_IsOK(val_res)) {
                    RuntimeError("cannot convert value of dictionary"); 
                }

                CNTK::ValuePtr* cntk_value = reinterpret_cast<CNTK::ValuePtr*>(cntk_val);

                $1[it.first] = *cntk_value;
            }
        }
    }
}
...
namespace CNTK {
    ...
    class UserBackPropState : public BackPropState {
        public:
        UserBackPropState(const FunctionPtr& function, const DeviceDescriptor& computeDevice, int userData)
